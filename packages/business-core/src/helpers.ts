...
export function isEqualValue(left: unknown, right: unknown): boolean {
  ...
  if (isPlainObject(left) && isPlainObject(right)) {
    ...
    return leftKeys.length === rightKeys.length && leftKeys.every(
      (key) => Object.prototype.hasOwnProperty.call(right, key) && isEqualValue(left[key], right[key])
    )
  }
  return false
}
...
export function getByPath(source: unknown, path: string): unknown {
  ...
  return path.replace(/\[(\w+)\]/g, '.$1').split('.').filter(Boolean).reduce<unknown>((value, key) => {
...
export function matchesFileAccept(file: Pick<File, 'name' | 'type'>, accept: string): boolean {
  ...
  return rules.some((rule) => {
    ...
    if (rule.endsWith('/*')) {
      ...
      return rule === 'image/*' && ['.apng', '.avif', '.bmp', '.gif', '.ico', '.jpeg', '.jpg', '.png', '.svg', '.webp'].includes(extension)
    }
    return type === rule
  })
}
...
function matchesRequirement(requirement: PermissionRequirement | undefined, available: string[], match: PermissionMatchMode): boolean {
  const expected = toPermissionValues(requirement)
  if (expected.length === 0) return true
  return match === 'all' ? expected.every((item) => available.includes(item)) : expected.some((item) => available.includes(item))
}
...
export function evaluatePermission(options: PermissionDirectiveValue = {}, provider: PermissionProvider = {}): boolean {
  ...
  const context: PermissionCheckContext = { permission: options.permission, roles: options.roles, match, permissions, currentRoles }
  ...
  const permissionGranted = !options.permission || superPermissions.some((item) => permissions.includes(item)) || matchesRequirement(options.permission, permissions, match)
  const roleGranted = !options.roles || superRoles.some((item) => currentRoles.includes(item)) || matchesRequirement(options.roles, currentRoles, match)
