...
const emit = defineEmits<{
  'update:model': [model: QueryFormModel]
  query: [model: QueryFormModel]
  reset: [model: QueryFormModel]
}>()
...
const columnsPerRow = computed(() =>
  Math.max(1, Number({ ...defaults, ...props.breakpointCols }[activeBreakpoint.value]) || 1)
)
...
const normalizedEdge = (value: unknown) =>
...
const fieldRows = computed(() => {
  ...
  visibleFields.value.forEach((field, index) => {
    ...
    if (row.length && used + span > 24) {
      rows.push(row)
      row = []
      used = 0
    }
    row.push({ field, span, index })
    used += span
  })
...
const shouldShowToggle = computed(
  () => props.showToggle && fieldRows.value.length > normalizedMaxRows.value
)
const visibleRows = computed(() =>
  !shouldShowToggle.value || expanded.value
    ? fieldRows.value
    : fieldRows.value.slice(0, normalizedMaxRows.value)
)
watch(shouldShowToggle, (value) => {
...
function componentProps(field: QueryFormField) {
  ...
  if (
    !Object.prototype.hasOwnProperty.call(values, 'clearable') &&
    clearableComponents.includes(name)
  )
    values.clearable = true
  return values
}
function handleQuery() {
  emit('query', cloneValue(innerModel.value))
}
function reset() {
...
function handleKeyup(event: KeyboardEvent) {
  if (event.key === 'Enter' || event.keyCode === 13) handleQuery()
}
const resize = () => {
  ...
  <div class="x-query-form query-form">
    ...
    >
      ...
      >
        <el-col
          v-for="item in row"
          :key="item.field.prop || item.field.slotName || item.index"
          :span="item.span"
        >
          <el-form-item
            :label="item.field.label"
            :prop="splitRange(item.field) ? startProp(item.field) : item.field.prop"
          >
            <div
              class="query-form__control"
              :class="{ 'query-form__control--radio-group': item.field.layout === 'radio-group' }"
              @keyup="handleKeyup"
            >
              <slot
      ...
      <div v-if="showActions || shouldShowToggle" class="query-form__footer">
        <div class="query-form__action-bar">
          <el-button
            ...
            :icon="Search"
            @click="handleQuery"
            >{{ queryText || business.t('query.search') }}</el-button
          >
          <el-button v-if="showActions" size="small" :icon="Refresh" @click="reset">{{
            resetText || business.t('query.reset')
          }}</el-button>
          <el-button
          ...
          >
            {{
              expanded
                ? collapseText || business.t('query.collapse')
                : expandText || business.t('query.expand')
