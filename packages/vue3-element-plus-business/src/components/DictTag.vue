...
const emit = defineEmits<{ click: [option: BusinessDictOption | undefined, value: DictValue, event: MouseEvent] }>()
defineSlots<{ default?: (scope: { option?: BusinessDictOption; value: DictValue; label: string }) => unknown }>()
const empty = (value: unknown) => value === undefined || value === null || value === ''
const values = computed<DictValue[]>(() => Array.isArray(props.value)
  ? props.value.filter((item) => !empty(item))
  : empty(props.value) ? [] : [props.value as DictValue])
const optionFor = (value: DictValue) => props.options.find((option) => props.strict ? option.value === value : String(option.value) === String(value))
const labelFor = (value: DictValue, option?: BusinessDictOption) => option?.label ?? (typeof props.fallback === 'function' ? String(props.fallback(value)) : props.fallback ?? String(value))
</script>
    ...
    <template v-for="(item, index) in values" :key="`${item}-${index}`">
      ...
      <slot :option="optionFor(item)" :value="item" :label="labelFor(item, optionFor(item))">
        <el-tag
          ...
          :class="optionFor(item)?.className || optionFor(item)?.elTagClass || optionFor(item)?.listClass"
