...
const props = withDefaults(
  ...
  }>(),
  {
    ...
    total: 0,
    showPagination: true,
    paginationHiddenWhenNoData: true,
    paginationProps: () => ({})
  }
)
...
const shouldShowPagination = computed(
  () => props.showPagination && (!props.paginationHiddenWhenNoData || props.total > 0)
)
const tableSize = computed(() =>
  props.size === 'medium' ? 'default' : props.size === 'mini' ? 'small' : props.size
)
const reserved = new Set(['visible', 'slotName', 'headerSlotName', 'columnSetting', 'key'])
function columnProps(column: ProTableColumn) {
  const result = Object.fromEntries(
    Object.entries(column).filter(([key, value]) => !reserved.has(key) && value !== undefined)
  )
  if (result.showOverflowTooltip === undefined)
    result.showOverflowTooltip = column.type ? false : true
  return result
}
